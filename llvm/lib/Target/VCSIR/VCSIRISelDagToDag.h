//===---- VCSIRISelDAGToDAG.h - A dag to dag inst selector for VCSIR -----===//
//
...
//===----------------------------------------------------------------------===//
//
// This file defines an instruction selector for the VCSIR target.
//
//===----------------------------------------------------------------------===//

#ifndef LLVM_LIB_TARGET_VCSIR_VCSIRISELDAGTODAG_H
#define LLVM_LIB_TARGET_VCSIR_VCSIRISELDAGTODAG_H
...
// VCSIR specific code to select VCSIR machine instructions for
// SelectionDAG operations.
namespace llvm {
class VCSIRDAGToDAGISel : public SelectionDAGISel {
  const VCSIRSubtarget *Subtarget = nullptr;

public:
  explicit VCSIRDAGToDAGISel(VCSIRTargetMachine &TargetMachine,
                             CodeGenOptLevel OptLevel)
      : SelectionDAGISel(TargetMachine, OptLevel) {}

  bool runOnMachineFunction(MachineFunction &MF) override {
    Subtarget = &MF.getSubtarget<VCSIRSubtarget>();
  ...
  // Return the VCSIR condition code that matches the given DAG integer
  // condition code. The CondCode must be one of those supported by the VCSIR
  // ISA (see translateSetCCForBranch).
  static VCSIRCC::CondCode getVCSIRCCForIntCC(ISD::CondCode CC) {
    switch (CC) {
    default:
...
class VCSIRDAGToDAGISelLegacy : public SelectionDAGISelLegacy {
public:
  static char ID;
  explicit VCSIRDAGToDAGISelLegacy(VCSIRTargetMachine &TargetMachine,
                                   CodeGenOptLevel OptLevel);
};
