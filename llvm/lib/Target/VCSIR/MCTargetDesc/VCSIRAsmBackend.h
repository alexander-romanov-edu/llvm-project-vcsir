//===-- VCSIRAsmBackend.h - VCSIR Assembler Backend ----------------------===//
//
...
#ifndef LLVM_LIB_TARGET_VCSIR_MCTARGETDESC_VCSIRASMBACKEND_H
#define LLVM_LIB_TARGET_VCSIR_MCTARGETDESC_VCSIRASMBACKEND_H
...
class VCSIRAsmBackend : public MCAsmBackend {
  const MCSubtargetInfo &STI;
  uint8_t OSABI;
  bool ForceRelocs = false;

public:
  VCSIRAsmBackend(const MCSubtargetInfo &STI, uint8_t OSABI)
      : MCAsmBackend(llvm::endianness::little), STI(STI), OSABI(OSABI) {}
  ~VCSIRAsmBackend() override = default;
  ...
  unsigned getNumFixupKinds() const override {
    return VCSIR::NumTargetFixupKinds;
  }
