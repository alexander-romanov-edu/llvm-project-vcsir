//===-- VCSIRCallLowering.cpp - Call lowering -------------------*- C++ -*-===//
//
...
struct VCSIROutgoingValueAssigner : public CallLowering::OutgoingValueAssigner {
private:
  // The function used internally to assign args - we ignore the AssignFn stored
  // by OutgoingValueAssigner since VCSIR implements its CC using a custom
  // function with a different signature.
  VCSIRCCAssignFn *VCSIRAssignFn;

  // Whether this is assigning args for a return.
  bool IsRet;

public:
  VCSIROutgoingValueAssigner(VCSIRCCAssignFn *VCSIRAssignFn_, bool IsRet)
      : CallLowering::OutgoingValueAssigner(nullptr),
        VCSIRAssignFn(VCSIRAssignFn_), IsRet(IsRet) {}

  bool assignArg(unsigned ValNo, EVT OrigVT, MVT ValVT, MVT LocVT,
                 CCValAssign::LocInfo LocInfo,
                 const CallLowering::ArgInfo &Info, ISD::ArgFlagsTy Flags,
                 CCState &State) override {
    if (VCSIRAssignFn(ValNo, ValVT, LocVT, LocInfo, Flags, State, Info.IsFixed,
                      IsRet, Info.Ty))
      return true;
...
struct VCSIROutgoingValueHandler : public CallLowering::OutgoingValueHandler {
  VCSIROutgoingValueHandler(MachineIRBuilder &B, MachineRegisterInfo &MRI,
                            MachineInstrBuilder MIB)
      : OutgoingValueHandler(B, MRI), MIB(MIB),
        Subtarget(MIRBuilder.getMF().getSubtarget<VCSIRSubtarget>()) {}
  Register getStackAddress(uint64_t MemSize, int64_t Offset,
    ...
    if (!SPReg)
      SPReg = MIRBuilder.buildCopy(p0, Register(VCSIR::X2)).getReg(0);
...
struct VCSIRIncomingValueAssigner : public CallLowering::IncomingValueAssigner {
private:
  // The function used internally to assign args - we ignore the AssignFn stored
  // by IncomingValueAssigner since VCSIR implements its CC using a custom
  // function with a different signature.
  VCSIRCCAssignFn *VCSIRAssignFn;
...
public:
  VCSIRIncomingValueAssigner(VCSIRCCAssignFn *VCSIRAssignFn_, bool IsRet)
  ...
  bool assignArg(unsigned ValNo, EVT OrigVT, MVT ValVT, MVT LocVT,
    ...
    if (VCSIRAssignFn(ValNo, ValVT, LocVT, LocInfo, Flags, State,
...
struct VCSIRIncomingValueHandler : public CallLowering::IncomingValueHandler {
  VCSIRIncomingValueHandler(MachineIRBuilder &B, MachineRegisterInfo &MRI)
...
struct VCSIRFormalArgHandler : public VCSIRIncomingValueHandler {
  VCSIRFormalArgHandler(MachineIRBuilder &B, MachineRegisterInfo &MRI)
      : VCSIRIncomingValueHandler(B, MRI) {}
...
struct VCSIRCallReturnHandler : public VCSIRIncomingValueHandler {
  VCSIRCallReturnHandler(MachineIRBuilder &B, MachineRegisterInfo &MRI,
                         MachineInstrBuilder &MIB)
      : VCSIRIncomingValueHandler(B, MRI), MIB(MIB) {}
...
VCSIRCallLowering::VCSIRCallLowering(const VCSIRTargetLowering &TLI)
    : CallLowering(&TLI) {}

static bool isSupportedArgumentType(Type *T, const VCSIRSubtarget &Subtarget,
...
static bool isSupportedReturnType(Type *T, const VCSIRSubtarget &Subtarget,
...
bool VCSIRCallLowering::lowerReturn(MachineIRBuilder &MIRBuilder,
                                    const Value *Val, ArrayRef<Register> VRegs,
                                    FunctionLoweringInfo &FLI) const {
  assert(!Val == VRegs.empty() && "Return value without a vreg");
  MachineInstrBuilder Ret = MIRBuilder.buildInstrNoInsert(VCSIR::PseudoRET);

  if (!FLI.CanLowerReturn) {
    insertSRetStores(MIRBuilder, Val->getType(), VRegs, FLI.DemoteRegister);
  } else if (!VRegs.empty()) {
    const VCSIRSubtarget &Subtarget =
        MIRBuilder.getMF().getSubtarget<VCSIRSubtarget>();
    ...
    VCSIROutgoingValueAssigner Assigner(
        CC == CallingConv::Fast ? CC_VCSIR_FastCC : CC_VCSIR,
        /*IsRet=*/true);
    VCSIROutgoingValueHandler Handler(MIRBuilder, MF.getRegInfo(), Ret);
...
bool VCSIRCallLowering::canLowerReturn(MachineFunction &MF,
  ...
  for (unsigned I = 0, E = Outs.size(); I < E; ++I) {
    ...
    if (CC_VCSIR(I, VT, VT, CCValAssign::Full, Outs[I].Flags[0], CCInfo,
...
void VCSIRCallLowering::saveVarArgRegisters(
...
bool VCSIRCallLowering::lowerFormalArguments(MachineIRBuilder &MIRBuilder,
                                             const Function &F,
  ...
  VCSIRIncomingValueAssigner Assigner(CC == CallingConv::Fast ? CC_VCSIR_FastCC
                                                              : CC_VCSIR,
                                      /*IsRet=*/false);
  VCSIRFormalArgHandler Handler(MIRBuilder, MF.getRegInfo());
...
bool VCSIRCallLowering::lowerCall(MachineIRBuilder &MIRBuilder,
  ...
  MachineInstrBuilder CallSeqStart =
      MIRBuilder.buildInstr(VCSIR::ADJCALLSTACKDOWN);
  ...
  MachineInstrBuilder Call =
      MIRBuilder.buildInstrNoInsert(VCSIR::PseudoCALL).add(Info.Callee);
  const TargetRegisterInfo *TRI = Subtarget.getRegisterInfo();
  Call.addRegMask(TRI->getCallPreservedMask(MF, Info.CallConv));

  VCSIROutgoingValueAssigner ArgAssigner(
      CC == CallingConv::Fast ? CC_VCSIR_FastCC : CC_VCSIR,
      /*IsRet=*/false);
  VCSIROutgoingValueHandler ArgHandler(MIRBuilder, MF.getRegInfo(), Call);
  ...
  MIRBuilder.buildInstr(VCSIR::ADJCALLSTACKUP)
  ...
  if (Info.CanLowerReturn && !Info.OrigRet.Ty->isVoidTy()) {
    ...
    VCSIRIncomingValueAssigner RetAssigner(
        CC == CallingConv::Fast ? CC_VCSIR_FastCC : CC_VCSIR,
        /*IsRet=*/true);
    VCSIRCallReturnHandler RetHandler(MIRBuilder, MF.getRegInfo(), Call);
